...
`MiddlewareExecutor` es el componente responsable de ejecutar los middlewares registrados en el sistema, dependiendo de si el modo REST está habilitado (`ENABLE_REST`) o si se trata de una clase de excepción (`CLASS-EXCEPTIONS`), este ejecutor decide qué conjunto de middlewares aplicar.
...
Para registrar nuevos middlewares, debes sobrescribir el método `getMiddlewareExecutor()` en tu clase `Repository.php`:
